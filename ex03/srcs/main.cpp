...
/*   Updated: 2022/06/03 10:32:39 by jmaia            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	}
...

	{
		...
		megaVogon.signForm(*form);
		megaVogon.executeForm(*form);
		delete form;
		form = intern.makeForm("shrubbery creation", "Carol");
		megaVogon.signForm(*form);
		megaVogon.executeForm(*form);
		delete form;
		form = intern.makeForm("presidential pardon", "Dave");
		megaVogon.signForm(*form);
		megaVogon.executeForm(*form);
		delete form;

		form = intern.makeForm("pouet", "Oggy"); // This should print an error
	}
}
