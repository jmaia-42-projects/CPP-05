...
/*   Updated: 2022/05/31 16:00:01 by jmaia            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(std::string const name, unsigned int const signMinGrade, unsigned int const execMinGrade): name(name), isSigned(false), signMinGrade(signMinGrade), execMinGrade(execMinGrade)
{
...
void	Form::checkCanExecute(Bureaucrat const &executor) const
{
	if (!this->isSigned)
		throw (Form::FormNotSignedException());
	if (executor.getGrade() > this->execMinGrade)
...
void	Form::FormAlreadySignedException::printException(void)
{
	std::cerr << "Form has already been signed" << std::endl;
}

Form::FormNotSignedException::FormNotSignedException(void) { }

void	Form::FormNotSignedException::printException(void)
{
